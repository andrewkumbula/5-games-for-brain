/**
 * ВНИМАНИЕ: рабочая копия логики встроена в hub.js (один запрос). Этот файл — дубликат для правок/диффов.
 ...
 * Жёсткий потолок по числу открытых клеток — иначе 2–3 частые буквы открывают почти всю фразу.
 */
...
async function loadPhrasesCatalog() {
  ...
  for (const url of candidates) {
    try {
      ...
      if (Array.isArray(data.phrases) && data.phrases.length) return data.phrases;
    } catch {
      /* next */
    }
  }
...
async function bootCryptogram() {
  ...
  try {
    ...
    const entry = pickPhraseForDate(phrases, playDate);
    if (!entry) {
